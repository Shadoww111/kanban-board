...
export default function Login() {
  ...
  const go = async (e) => { e.preventDefault(); setBusy(true); try { await login(email, pw); nav("/"); } catch (err) { toast.error(err.response?.data?.msg || "failed"); } finally { setBusy(false); } };
  return (<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 to-purple-600 px-4"><div className="max-w-sm w-full">
    ...
    <form onSubmit={go} className="bg-white p-6 rounded-2xl shadow-xl space-y-4">
      ...
      <input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="password" required className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none"/>
      <button disabled={busy} className="w-full bg-indigo-600 text-white py-2.5 rounded-xl font-medium hover:bg-indigo-700 disabled:opacity-50">{busy?"...":"sign in"}</button>
    </form>
    <p className="text-center text-sm text-white/80 mt-4">no account? <Link to="/register" className="text-white font-medium hover:underline">register</Link></p>
  </div></div>);
}
